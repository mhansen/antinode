...
function find_local_file_path(req) {
    ...
    //remove anything after the colon in host header.
    var host_header = req.headers.host.split(':')[0];

    
    var vhost = settings.hosts[host_header] || settings.default_host;
    log.debug("requested host:", host_header, "selected vhost:", vhost.root);
